...
As of 2026-04-14, the backend is substantially implemented:
...
- **Phase 6 (Dreams Engine) — SHIPPED 2026-04-15.** Runtime surface lives at `src/starry_lyfe/dreams/`. Nightly apscheduler daemon (`python -m starry_lyfe.dreams`) plus per-character content generators (5 generators: schedule deterministic from `routines.yaml`; diary, off_screen, open_loops, activity_design all LLM-backed end-to-end with anti-contamination per-character system prompts) are landed. Retroactive Phase G prose rendering + Phase A'' Alicia-away communication_mode tagging + Phase H regression bundle all wired. The Dreams write/retrieve lifecycle and DB-backed assembler consumer path are green; automatic Scene Director `activity_context` population is auto-populated by Phase 7's HTTP service via the `MemoryBundle.activities` consumer path. Most rigorously audited phase in project history: 3 Codex audit rounds + 2 Claude Code remediation rounds + 2 Codex direct doc remediation passes + 1 Claude AI Step 5 QA pass with 1 inline direct remediation. See `Docs/_phases/PHASE_6.md` for the complete cycle record.
- **Phase 7 (HTTP service on port 8001) — SHIPPED 2026-04-15; post-ship audit remediation SHIPPED 2026-04-15; Round 2 re-audit remediation SHIPPED 2026-04-15; Direct Codex R3 remediation SHIPPED 2026-04-15.** Runtime surface lives at `src/starry_lyfe/api/`. FastAPI app factory (`create_app`) + uvicorn boot (`python -m starry_lyfe.api`) + 5 endpoints (`/health/live`, `/health/ready`, `/v1/models`, `/v1/chat/completions` with SSE streaming + `X-API-Key` auth, `/metrics` Prometheus exposition). Full 12-step request flow per §10 implemented in `api/orchestration/pipeline.py` — Step 9 Crew sequencing (multi-speaker SSE via `_run_crew_turn` looping `select_next_speaker()`) landed in the R1 F1 remediation; the Step 9 validated-output carry-forward contract from §7 (*"Adelia's validated turn becomes the input context for Reina's generation"*) landed in the R2 remediation via `_format_crew_prior_block`; R3-F1 direct remediation tightened the carry-forward to validated-only output (FAIL'd text is no longer injected into subsequent speakers). Closes the deferred Phase 6 → Phase 7 Dreams glue: Dreams-written `Activity` rows auto-populate Layer 6 on the next chat turn (verified by `tests/integration/test_http_dreams_glue.py`), and F2 remediation additionally wires `MemoryBundle.activities` into the Crew scorer's `NextSpeakerInput.activity_context`. `/health/ready` performs a live BD-1 HEAD probe (F3 closure). `http_sse_tokens_total` counter increments once per upstream LLM stream delta uniformly across both single-speaker and Crew paths (F5 + R2-F2 closures); help string reworded to "per labeled character (speaker in Crew mode, focal character otherwise)" in R3-F2. Post-turn fire-and-forget memory extraction + relationship evaluator (delta cap ±0.03 per CLAUDE.md §16) wired via `asyncio.create_task`. New `chat_sessions` table (Alembic migration 004). Test baseline: **1015 passed, 0 failed** (910 unit + 60 integration + 45 fidelity) post-Direct-R3-remediation. See `Docs/_phases/PHASE_7.md §10` (R1), `§12` (R2), `§14` (Direct R3) for the audit records.
- **Phase 8 (LLM Relationship Evaluator) — SEALED 2026-04-15.** First phase to adhere to `Docs/_phases/_TEMPLATE.md` six-step structure from the outset, closing the Phase 7 AC-7.20 governance gap going forward. Scope: swap the heuristic `_propose_deltas()` in `api/orchestration/relationship.py` for a BDOne-backed LLM evaluator without changing the public API or the ±0.03 cap (§7 Whyze-Byte Validation Pipeline crosscut). Runtime surface: new `src/starry_lyfe/api/orchestration/relationship_prompts.py` with canonical `RELATIONSHIP_EVAL_SYSTEM` (hand-authored per-character register sections for Adelia/Bina/Reina/Alicia drawn from the character kernels), `RelationshipEvalResponse` Pydantic schema active as the live validator via `model_validate` (R1-F2 closure), `build_eval_prompt(character_id, response_text)` (2-arg canonical shape ratified via R2a self-remediation) with `html.escape` injection defense on `response_text` (R1-F3 closure), and `parse_eval_response` with markdown-fence stripping + None-on-failure contract hardened against non-object JSON and JSON booleans (R1-F1 closure). `evaluate_and_update` gains `llm_client` + `settings` kwargs; LLM-primary with five heuristic-fallback branches (toggle off, missing client, circuit open, `DreamsLLMError`, parser None). Structured log events: `llm_eval_parsed_proposal` + `llm_eval_fallback_to_heuristic` (with `reason`). Three new `ApiSettings` fields: `relationship_eval_llm` (default True), `relationship_eval_max_tokens` (200), `relationship_eval_temperature` (0.2). Test baseline at ship: **1058 passed, 0 failed** (953 unit + 60 integration + 45 fidelity); +43 tests total over Phase 7 baseline. Step 5 QA verdict: APPROVED FOR SHIP, 15/15 ACs PASS, all 6 Codex findings closed (R1-F1 + R1-F2 + R1-F3 + R1-F4 Path B; R3-L1 + R3-L2 Path C). Governance: `Docs/_phases/PHASE_8.md` carries the full six-step record. **Commit chain:** `c773242` + `1cf85b2` + `f5c5cbd` + `feb8669` (Steps 1-2) + `6cc8533` + `6638825` + `39c8b53` + `2a62798` (Step 4 Round 1) + `486dae4` + `3400534` (Step 4'' Round 3 doc-only via Path C).
- **Phase 9 (DyadStateInternal LLM Evaluator) — Step 3'' Audit Round 3 COMPLETE 2026-04-15; Project Owner-authorized AGENTS.md Path C direct Codex doc-only remediation applied; gate PASS; ready for Step 5 QA.** Extends the Phase 8 LLM evaluator pattern from the 4 Whyze-dyads to the 6 inter-woman dyads tracked in `DyadStateInternal` (adelia×bina anchor_dynamic, bina×reina shield_wall marriage, adelia×reina kinetic_vanguard, plus 3 Alicia-orbital friendships). Adds a 5th dimension `conflict` to Phase 8's 4 (trust, intimacy, unresolved_tension, repair_history). Runtime surface: new `src/starry_lyfe/api/orchestration/internal_relationship_prompts.py` with `INTERNAL_RELATIONSHIP_EVAL_SYSTEM` carrying all 6 hand-authored per-pair register sections (verbatim from `PHASE_9.md §Pre-execution`); `InternalRelationshipEvalResponse` 5-field Pydantic schema reusing `_NumericValue` + `_reject_bool`; `build_internal_eval_prompt(dyad_key, member_a, member_b, response_text, *, speaker_id)` with `html.escape` injection defense and `Speaker:` line carrying focal speaker identity (R1-F1 closure 2026-04-15); `parse_internal_eval_response` with `isinstance(raw, dict)` fail-closed guard. New `src/starry_lyfe/api/orchestration/internal_relationship.py` with `InternalDyadDeltaProposal` (5 fields) + `InternalRelationshipUpdate` audit record + `_propose_internal_deltas` heuristic (extended with `_CONFLICT_POSITIVE` / `_CONFLICT_NEGATIVE` banks) + `_llm_propose_internal_deltas` (threads focal `character_id` to the prompt as `speaker_id`) + `evaluate_and_update_internal`. Reuses `_clamp_delta` + `_bound01` from `relationship.py` (zero duplication). `post_turn.py::schedule_post_turn_tasks` adds a third `asyncio.create_task` for the inter-woman evaluator after the Whyze-dyad evaluator (single task; fan-out happens internally via SQL filter). Alicia-orbital active-gate enforced at the SQL boundary via `is_currently_active.is_(True)` so dormant orbital dyads never reach the LLM call site (R1-F2 closure 2026-04-15: communication-mode-aware remote-turn handling described in canonical Pre-execution `Alicia-orbital note` blocks is explicitly deferred to a future phase). New `ApiSettings.internal_relationship_eval_llm` (default True); reuses Phase 8's `relationship_eval_max_tokens` + `relationship_eval_temperature`. Structured log events: `internal_llm_eval_parsed_proposal` + `internal_llm_eval_fallback_to_heuristic`. Test baseline: **1119 passed, 0 failed** (1014 unit + 60 integration + 45 fidelity); +61 tests total over Phase 8 baseline (35 prompts + 20 evaluator + 6 R1-F1 regression). Phase 8 R1-F1 / R1-F2 / R1-F3 lessons applied proactively in Step 2. Governance: `Docs/_phases/PHASE_9.md` carries the full Step 1 Plan + Step 2 Execute log + Step 3 Round 1 Codex audit + Step 4 Round 1 remediation record + Step 3'' Round 3 audit closure under Path C. **Step 2 commit chain:** `a3148f5` + `3449335` + `4b50132` (governance). **Step 4 Round 1 commit chain:** `b301b16` (RT1 F1 substantive code fix + regression suite) + `2906ed3` (RT2 F2 doc-narrow Alicia-orbital remote-turn) + `11a8af6` (RT3 governance sweep). Round 3 Path C direct Codex doc-only remediation closes the duplicate handshake row, stale `1118/+5/residue-failure` narrative, and remaining placeholder hashes in the Phase 9 governance surfaces.
...
- **Service surface (port 8001):** SHIPPED 2026-04-15 as Phase 7. Implementation lives at `src/starry_lyfe/api/`. Five endpoints (`/health/live`, `/health/ready`, `/v1/models`, `/v1/chat/completions` SSE streaming with `X-API-Key` auth, `/metrics`). Boot via `python -m starry_lyfe.api`. Full 12-step request flow per §10. See `Docs/_phases/PHASE_7.md`.
...
- **Dreams Engine:** COMPLETE as Phase 6. Implementation lives at `src/starry_lyfe/dreams/`; `run_dreams_pass()` is the public entry and `python -m starry_lyfe.dreams` starts the apscheduler daemon. The Dreams write/retrieve lifecycle and assembler consumer path are landed; automatic Scene Director `activity_context` population lands in two stages: Phase 7 ship (2026-04-15) closed the assembler Layer 6 consumer path (proof: `tests/integration/test_http_dreams_glue.py::test_dreams_activity_lands_in_layer_6`); Phase 7 post-ship audit F2 remediation (2026-04-15) wires `MemoryBundle.activities` into `NextSpeakerInput.activity_context` at the pipeline boundary (see `Docs/_phases/PHASE_7.md §10`). Architecture remains documented at §9.
...
**SHIPPED 2026-04-15** as Phase 6 of the overall backend build. Runtime surface lives at `src/starry_lyfe/dreams/`; the full phase record lives at `Docs/_phases/PHASE_6.md`.
...
Current scope note: Phase 6 closes the Dreams write/retrieve lifecycle and the DB-backed assembler consumer path. Automatic population of Scene Director `NextSpeakerInput.activity_context` from retrieved Dreams activities lands in two stages: Phase 7 ship (2026-04-15) closed the assembler Layer 6 consumer path — `tests/integration/test_http_dreams_glue.py::test_dreams_activity_lands_in_layer_6`; Phase 7 post-ship audit F2 remediation (2026-04-15) wires `MemoryBundle.activities` into the Crew scorer's `activity_context` input at the pipeline boundary — `src/starry_lyfe/api/orchestration/pipeline.py::_build_activity_context` + `_run_crew_turn`. F2 closure record in `Docs/_phases/PHASE_7.md §10`.
...
- **HTTP service on port 8001 — SHIPPED 2026-04-15 as Phase 7.** Implementation lives at `src/starry_lyfe/api/`; spec at `Docs/_phases/PHASE_7.md`.
