...
- **Phase 7 (HTTP service on port 8001) — SHIPPED 2026-04-15.** Runtime surface lives at `src/starry_lyfe/api/`. FastAPI app factory (`create_app`) + uvicorn boot (`python -m starry_lyfe.api`) + 5 endpoints (`/health/live`, `/health/ready`, `/v1/models`, `/v1/chat/completions` with SSE streaming + `X-API-Key` auth, `/metrics` Prometheus exposition). Full 12-step request flow per §10 implemented in `api/orchestration/pipeline.py`. Closes the deferred Phase 6 → Phase 7 Dreams glue: Dreams-written `Activity` rows auto-populate Layer 6 on the next chat turn (verified end-to-end by `tests/integration/test_http_dreams_glue.py`). Post-turn fire-and-forget memory extraction + relationship evaluator (delta cap ±0.03 per CLAUDE.md §16) wired via `asyncio.create_task`. New `chat_sessions` table (Alembic migration 004). Test baseline: **995 passed, 0 failed** (890 unit + 60 integration + 45 fidelity). See `Docs/_phases/PHASE_7.md` for the complete spec + acceptance trace.
