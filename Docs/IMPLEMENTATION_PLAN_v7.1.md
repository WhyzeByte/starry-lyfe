...
- **Phase 8 (LLM Relationship Evaluator) — Step 2 Execute COMPLETE 2026-04-15; pending Codex Step 3 Audit Round 1.** First phase to adhere to `Docs/_phases/_TEMPLATE.md` six-step structure from the outset, closing the Phase 7 AC-7.20 governance gap going forward. Scope: swap the heuristic `_propose_deltas()` in `api/orchestration/relationship.py` for a BDOne-backed LLM evaluator without changing the public API or the ±0.03 cap (§7 Whyze-Byte Validation Pipeline crosscut). Runtime surface: new `src/starry_lyfe/api/orchestration/relationship_prompts.py` with canonical `RELATIONSHIP_EVAL_SYSTEM` (hand-authored per-character register sections for Adelia/Bina/Reina/Alicia drawn from the character kernels), `RelationshipEvalResponse` Pydantic schema, `build_eval_prompt(character_id, response_text)` (2-arg canonical shape ratified via R2a self-remediation), and `parse_eval_response` with markdown-fence stripping + None-on-failure contract. `evaluate_and_update` gains `llm_client` + `settings` kwargs (both default None for backward compat); LLM-primary with five heuristic-fallback branches (toggle off, missing client, circuit open, `DreamsLLMError`, parser None). Structured log events: `llm_eval_parsed_proposal` + `llm_eval_fallback_to_heuristic` (with `reason`). Three new `ApiSettings` fields: `relationship_eval_llm` (default True), `relationship_eval_max_tokens` (200), `relationship_eval_temperature` (0.2). Test baseline: **1035 passed, 0 failed** (930 unit + 60 integration + 45 fidelity); +20 tests (13 in `test_relationship_prompts.py`, 7 in `test_relationship_evaluator.py::TestEvaluateAndUpdateLLMPath`). Governance: `Docs/_phases/PHASE_8.md` carries the full Step 1 Plan + Step 2 Execute log + handshake to Codex; 15/15 ACs MET post R1/R2a/R3b self-remediation. Commit chain: `c773242` (prompts module) + `1cf85b2` (evaluator rewire) + `f5c5cbd` (governance sweep).
