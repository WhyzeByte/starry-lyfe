...
**Shipped phases:** Phase 0, A, A', A'', B, C, D (2026-04-12), E (2026-04-13), F (2026-04-13), G (2026-04-13), J.1–J.4 (2026-04-13), H (2026-04-13), K (2026-04-13), Phase 4 (Whyze-Byte Validation Pipeline, 2026-04-13), Phase F-Fidelity (Positive Fidelity Test Harness, 2026-04-14), Phase 5 (Scene Director, 2026-04-14), Phase 6 (Dreams Engine, 2026-04-15), Phase 7 (HTTP Service on Port 8001, 2026-04-15), Phase 8 (LLM Relationship Evaluator, SEALED 2026-04-15), Phase 9 (Inter-woman DyadStateInternal LLM evaluator, SEALED 2026-04-16), **Phase 10.0–10.6 (YAML Source-of-Truth Migration, 2026-04-16)**: 10.0 gap audit + preserve_marker remediation RATIFIED; 10.1 schema+loader infrastructure; 10.2 kernel body + voice exemplars cutover; 10.3 soul essence cutover; 10.3b soul cards embedding + iterator rewire; 10.4 narrow canon constraint pillars + Phase 8/9 evaluator register sections rewired to YAML; 10.5 ARCHIVE retired markdown/Python sources to `Archive/v7.1_pre_yaml/` with SHA256 manifest; 10.6 schema enforcement test hardening — unconditional preserve_marker enforcement (62/62 pass) + Layer 1 assembled-output verification + 5 new invariant test files (voice mode coverage, constraint pillar shape, cross-references, perspective divergence, shared_canon purity) + normalization_notes promoted to canonical drift record. Lettered-phase remediation complete (2026-04-13). Phase doc housekeeping complete (2026-04-14).
**Canonical authorship surface (post-10.5):** The 5 rich per-character YAMLs at `Characters/{adelia_raye,bina_malek,reina_torres,alicia_marin,shawn_kroon}.yaml` plus `Characters/shared_canon.yaml` are the sole runtime-authoritative source for all canonical character content. Legacy markdown kernels, Voice files, Knowledge Stacks, Pair files, `soul_essence.py`, and 15 soul card markdowns are archived under `Archive/v7.1_pre_yaml/` with SHA256 manifest. Do NOT author new markdown character files. All character prose (kernel body, voice exemplars, soul essence, soul cards, evaluator registers, constraint pillars) lives in the per-character YAML.

**Open ship gate:** Phase 10.5b (narrow canon loader rewire — `load_all_canon()` constructs the 7 narrow Pydantic objects from rich YAML + shared_canon.yaml in-memory instead of reading the 6 narrow `src/starry_lyfe/canon/*.yaml` files). Codex Round 1 audit returned **FAIL** with 6 findings (F1 Critical PermissionError now transient-resolved; F2 High soul cards cutover now landed; F3 High Layer 5 pair metadata still on legacy `pairs_loader.py` — this IS 10.5b scope; F4 Medium schema/validator gaps; F5 Medium cache key missing rich-YAML mtime; F6 Low governance doc staleness — being remediated by Claude AI 2026-04-16). Deferred to future sub-phases: Phase 9 evaluator 6→12 per-POV register split (Phase 10.4b scope). Note: preserve_marker unconditional enforcement landed in Phase 10.6 (62/62 pass, `VOICE_JUDGMENT_MARKERS` exclusion set removed from `test_rich_loader.py`). The 5 voice-judgment preserve_marker items previously listed as deferred were **completed** by Claude AI on 2026-04-15 (62/62 pass — see `PHASE_10_GAP_AUDIT.md §3.3`).
**Next phase:** Step 4 remediation playbook authored by Claude AI 2026-04-16 in `PHASE_10.md`. Claude Code to execute RT1 (F3 Layer 5 pair metadata cutover to rich YAML — this IS Phase 10.5b), RT2 (F4 schema/validator hardening + all-six-dyads divergence enforcement), RT3 (F5 cache key mtime inclusion). On Codex Round 2 PASS → Claude AI Step 5 QA → Phase 10.7 (Dreams Consistency QA). Test baseline **1231 passed, 7 skipped, 6 xfailed, 0 failed** as of 2026-04-16 post-10.6. Msty AI is the only consumer.
...
2. **Kernel body** — `compile_kernel(character, budget)` returns the trimmable, budget-bounded kernel body from `Characters/{name}.yaml::kernel_sections` (post-10.2 YAML block scalars, 11 numbered sections per character). Legacy `Characters/{Name}_v7.1.md` markdown kernels archived 2026-04-16.

3. **Soul cards** — `Characters/{name}.yaml::soul_cards[]` (post-10.3b, 15 cards embedded: 4 pair always-activated for focal character in Layer 1 + 11 knowledge cards scene-conditional in Layer 6 via `scene_keyword`, `communication_mode`, `with_character`, `always`). Runtime API: `soul_cards.load_all_soul_cards()` iterates `RichCharacter.soul_cards`. Legacy `src/starry_lyfe/canon/soul_cards/*.md` markdowns archived 2026-04-16.
