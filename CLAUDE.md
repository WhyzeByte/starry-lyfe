...
**Open ship gate:** Phase 8 (LLM Relationship Evaluator) — **Step 4 Round 1 Remediation COMPLETE 2026-04-15** (all 4 Codex R1+R2 findings FIXED across 4 commits: parser fail-closed, Pydantic schema activation, prompt injection defense, governance sync); handshake to Codex for Round 3 re-audit. Spec: `Docs/_phases/PHASE_8.md`. Scope: swapped heuristic `_propose_deltas()` in `api/orchestration/relationship.py` for a BDOne-backed LLM evaluator; public API + ±0.03 cap preserved. First phase to adhere to `Docs/_phases/_TEMPLATE.md` six-step structure from the outset (closes the Phase 7 AC-7.20 governance gap going forward).
