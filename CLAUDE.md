...
**Version:** U1.3-P2.23 | **Format:** U{universal}.{minor}-P{project}.{minor}
...
- Architectural phases 4–7 shipped end-to-end; Phase 8/9 sealed; Phase 10.0–10.7 shipped and post-ship audit/remediation-synchronized; **Phase 11 (Cross-Persona Context Injection) shipped 2026-04-17 → Codex Round 1 audit FAIL → R1 REMEDIATION COMPLETE 2026-04-17** — pending Codex Round 2 re-audit. The inadequate self-audit recorded earlier is marked SUPERSEDED in PHASE_11.md §10 with the lesson captured (self-audit ≠ independent audit). R1 remediation closed all 4 Codex findings: F1 stale Crew test suite (4-passed), F2 prompt-frame injection bypass (new `_sanitize_prior_text` helper + 3 adversarial regression tests), F3 unbounded aggregate prior-frame size (new `_PRIOR_FRAME_TOTAL_CHAR_CAP` + `_apply_aggregate_cap` + 1 regression test), F4 phase-record gate-state (status reopen + new §12/§13/§14). Post-audit operational fixes (Bearer auth `40f72cc`, containerization `d8672f3`, `.env` scaffolding `f6af9fd`) recorded honestly in PHASE_11.md §13. Phase 11 closes the architectural track for the v7 build; AC-11.8 awaits Project Owner Step 6 ship sign-off after Codex Round 2 PASS.
