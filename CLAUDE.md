...
**Open ship gate:** **Phase 10.5c (narrow canon loader rewire)** — `load_all_canon()` currently still reads the 7 narrow `src/starry_lyfe/canon/*.yaml` files directly (characters, pairs, dyads, interlocks, protocols, routines, voice_parameters). Phase 10.5c will rewire `load_all_canon()` to construct the 7 narrow Pydantic objects from rich YAML + `shared_canon.yaml` in-memory, at which point those 7 narrow YAMLs become archive candidates. Named consumers to migrate: `db/seed.py`, `api/app.py`, `context/assembler.py`, `dreams/daemon.py`, `canon/validator.py`, `context/layers.py`. Dedicated plan required before execution.

**Next phase:** Codex Round 3 audit on the 10.5b RT1/RT2/RT3 + R2-F1/F2/F3/F4 remediation surface. On PASS → Claude AI Step 5 QA → Project Owner ships Phase 10 → Phase 10.5c (plan + ship) → Phase 10.7 (Dreams Consistency QA per `C:\Users\Whyze\.claude\plans\plan-phase-10-7-sprightly-thunder.md`). Test baseline **1254 passed, 0 failed, 0 skipped, 0 xfailed** expected after R2 remediation (1251 post-10.5b + 2 new R2-F1 OneDrive resilience tests + 1 new R2-F3 sentinel probe). Final live figure recorded on ship commit. Msty AI is the only consumer.
